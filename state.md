...
## CSV schema (final — weight_kg required, unit_weights optional)

`id,area,priority,weight_kg,unit_weights`

- `weight_kg` = the normal total package weight — always present, used directly as `TotalWeight` (not summed from anything). This is the assignment's original field, unchanged in meaning.
- `unit_weights` = OPTIONAL semicolon-separated list of each individual unit's actual weight (e.g. `17;1` for two units weighing 17kg and 1kg). Left blank for every normal delivery. Only needed when a request is a bundle that might require splitting.

**Why not derive `TotalWeight` by summing `unit_weights` (earlier version did this):** most deliveries are simple single items that never need a unit breakdown at all — computing a sum just to get back the same one number every time added no value. Storing `weight_kg` directly is simpler and matches the assignment's original format exactly; `unit_weights` is purely additional detail, only read when splitting is actually attempted.

**Known trade-off, stated in README:** `weight_kg` and `unit_weights` are not cross-validated against each other when both are present. If they disagreed in a hand-edited file, the mismatch wouldn't be caught. Accepted as a trusted-input assumption rather than adding validation code for a case the assignment doesn't require handling.

## Extra feature — splittable overweight packages (final — unit-level bin packing, weight_kg-gated)

Instead of blanket-declining every delivery over the 10kg capacity:

1. If `weight_kg <= 10kg` → passes through unchanged. `unit_weights` is not even looked at in this case, whether or not it's populated.
2. If `weight_kg > 10kg` and `unit_weights` is empty (no breakdown provided) → cannot split, no way to know how to divide it → excluded and reported.
3. If `weight_kg > 10kg` and `unit_weights` is provided → split at the unit level: units that individually exceed capacity (**can never ship, ever** — excluded and reported one by one) vs. units that individually fit.
4. The fitting units are bin-packed into as few "part" deliveries as possible using **first-fit-decreasing**: sort them largest-first, place each into the first part with room, else start a new part. Each part's `TotalWeight` is the sum of its own units. Parts are suffixed ids (e.g. `3-part1`, `3-part2`), same area/priority as the original — ordinary deliveries from that point on, flowing through the normal tier + area packing algorithm with no special-casing needed there.
...
| id | area | priority | weight_kg | unit_weights | purpose |
|----|------|----------|-----------|--------------|---------|
| 1-5 | mixed | mixed | mixed | (blank) | assignment's original sample data (normal case) |
| 6 | Maadi | 1 | `3.0` | (blank) | same area + same priority as row 2 → should cluster into the same trip |
| 7 | Giza | 2 | `12.0` | (blank) | over capacity, no unit breakdown provided → unsplittable, excluded & reported |
| 8 | Heliopolis | 3 | `20.0` | `5;5;5;5` | 4 equal-weight units → splits into two full 10kg parts |
| 9 | Dokki | 1 | `30.0` | `15;15` | both units individually exceed capacity → fully unsplittable, excluded & reported |
| 10 | Nasr City | 2 | `18.0` | `17;1` | non-uniform units — the 17kg unit is excluded & reported individually, the 1kg unit still ships normally |
